...
from `ZenRailsBaseApp` to your application name, in camel case.
    ...
    replace the following strings:
        - Replace `mood_tracker` by `your_app_name` 
        - Replace `mood_tracker` by `YOUR_APP_NAME`
        - Replace `Mood Tracker` by the project's name
...
create the following seed users:
    - Ordinary user: email: `user@test.com` / password: `F00barbaz`
    - Admin user: email: `bwoodreid@gmail.com` / password: `F00barbaz`
